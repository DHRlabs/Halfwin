...
final class ClipboardHistory {
    ...
    private static let passwordManagerBundleIdentifiers: Set<String> = [
    ...
    func capture(from pasteboard: NSPasteboard) {
        ...
        if types.contains(urlType) {
            if let value = pasteboard.string(forType: urlType), let url = URL(string: value) {
                entry = ClipboardEntry(content: .url(url))
            } else if let urls = pasteboard.readObjects(forClasses: [NSURL.self], options: nil) as? [NSURL],
                      let url = urls.first {
                entry = ClipboardEntry(content: .url(url as URL))
            }
        }
        if entry == nil, let text = pasteboard.string(forType: .string), !text.isEmpty {
            entry = ClipboardEntry(content: .text(text))
        }
        guard pasteboard.changeCount == changeCount, let entry else { return }
        insert(entry)
    }
    ...
    func cancelPicker(runCancelAction: Bool = true) {
        guard isPickerVisible else { return }
        let action = onCancel
        dismissPicker()
        if runCancelAction { action?() }
    }

    func dismissPickerIfOutside(_ point: NSPoint) {
        guard isPickerVisible, let picker, !picker.frame.contains(point) else { return }
        cancelPicker()
    }
